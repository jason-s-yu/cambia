...
Both training modes read from a YAML config file. Example configs:

- `config/parallel.config.yaml` -- 23 workers, log archiving, short max turn count
- `config/serial.config.yaml` -- single worker, minimal logging
- `config/deep_train.yaml` -- Deep CFR training defaults
- `config/go_train.yaml` -- Go engine backend with threading
- `config/production_train.yaml` -- Production Deep CFR (GPU, large buffers)
- `config/bench.config.yaml` -- Benchmark configuration
...
| `pipeline_training` | bool | true | -- | Overlap next iteration's traversals with current strategy network training (6–12% wall-clock savings) |
| `sampling_method` | str | `"outcome"` | -- | Traversal sampling: `"outcome"` or `"external"` |
| `exploration_epsilon` | float | 0.6 | -- | Exploration parameter for outcome sampling (ignored for external) |
| `use_amp` | bool | false | -- | Automatic mixed precision (FP16) on CUDA |

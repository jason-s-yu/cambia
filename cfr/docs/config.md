...
* **`es_validation_interval`**:
  ...
  * Type: `integer`
  * Default: `10`
  * Example: `50`, `0` (disabled)
* **`es_validation_depth`**:
  * Description: Maximum game depth (in decision nodes) for each ES validation traversal. Shallower depths complete faster but give less accurate exploitability estimates. This is independent of the main training traversal depth.
  * Type: `integer`
  * Default: `10`
  * Example: `5`, `20`
* **`es_validation_traversals`**:
  * Description: Number of ES traversals to run per validation call. Higher counts reduce variance in the regret and entropy estimates at the cost of more wall-clock time per validation step.
  * Type: `integer`
  * Default: `1000`
  * Example: `500`, `2000`
* **`pipeline_training`**:
  * Description: If `true`, overlaps the next iteration's traversals with the current iteration's strategy network training phase. Traversals run concurrently via `ProcessPoolExecutor` (spawn context) while the strategy network trains on the main process. Provides 6–12% wall-clock savings at zero correctness risk.
  * Type: `boolean`
  * Default: `true`
* **`use_amp`**:
  * Description: If `true` and the device is CUDA, enables automatic mixed precision (AMP / FP16) during network training via `torch.amp.GradScaler` and `torch.amp.autocast`. For the current 175K-parameter network (256-wide hidden layers), benchmarks show AMP is 14–81% *slower* because Tensor Cores require layers >= 1024 units to be beneficial. Only enable this if the network grows beyond ~1M parameters.
  * Type: `boolean`
  * Default: `false`
  * CLI Override: `--amp` / `--no-amp`
* **`use_compile`**:
  * Description: If `true` and the device is CUDA, applies `torch.compile(mode="reduce-overhead")` to both the advantage and strategy networks for CUDA graph optimization. Expect a 30–90s warmup on the first training step. Only beneficial for long training runs where the warmup cost is amortized.
  * Type: `boolean`
  * Default: `false`

...
ctypes wrapper over `libcambia.so`, providing Python access to the Go game engine and agent state.

### Library Loading

The shared library is loaded once at module import time as a module-level singleton. The path is resolved in this order: the `LIBCAMBIA_PATH` environment variable, then the default location `cfr/libcambia.so`. If the library is not found, import raises `FileNotFoundError` with a message pointing to `make libcambia`. This means importing `bridge.py` in a worker process will fail fast if the build step was skipped, rather than failing silently mid-traversal.

Build dependency: `make libcambia` from the repo root. This compiles `engine/cgo/exports.go` into a shared library and copies it to `cfr/libcambia.so`.

### `GoEngine`

Drop-in replacement for `CambiaGameState` in the deep worker.
...
class GoEngine:
    ...
    def legal_actions_mask(self) -> np.ndarray      # (146,) bool
    def is_terminal(self) -> bool
    def get_utility(self) -> List[float]
    def save(self) -> int                            # returns snapshot handle
    def restore(self, handle: int) -> None
    def free_snapshot(self, handle: int) -> None
```

The save/restore interface replaces the apply/undo pattern used by the Python engine. Before recursing into a child node, the worker calls `save()` to capture the full game state (~200 bytes flat copy on the Go side). After returning from the child, it calls `restore(handle)` instead of undoing. Snapshot handles are integers into a fixed-size pool (256 slots) in the Go layer. The worker is responsible for calling `free_snapshot` after the snapshot is no longer needed.

`legal_actions_mask()` calls the Go engine's bitmask generator directly and converts the result to a `(146,)` numpy bool array. This avoids constructing `GameAction` objects in Python for the purpose of building a mask, which is the hot path during traversal.

### `GoAgentState`

Drop-in replacement for `AgentState` in the deep worker.
...
class GoAgentState:
    ...
    def encode(
        ...
        decision_context: DecisionContext,
        drawn_card_bucket: Optional[CardBucket] = None,
    ) -> np.ndarray                                  # (222,) float32
    def free(self) -> None
```

Agent handles are managed through a fixed-size pool (512 slots) on the Go side. `clone()` allocates a new handle and copies belief state. `free()` releases the handle back to the pool. The deep worker calls `free()` on cloned agent states after each traversal branch returns. `encode()` returns the same 222-dim tensor layout as `encoding.py`'s `encode_infoset()`.

### How It Connects to Other Modules

```text
ffi/bridge.py
  ├── libcambia.so: game_new, game_apply_action, game_clone, game_restore,
  │                 game_legal_actions, game_is_terminal, game_get_utility,
  │                 agent_new, agent_update, agent_clone, agent_encode
...
The validator accepts the same `engine_backend` config flag as the main training loop. With `engine_backend: go`, it imports `GoEngine` and `GoAgentState` from `ffi/bridge.py`. The import is deferred (inside `compute_exploitability`) so that the validator can be constructed without the library present; it only fails if `engine_backend: go` is actually used. With `engine_backend: python`, the fallback is the standard Python game engine with no import dependency on the FFI bridge.

### Integration with DeepCFRTrainer

`DeepCFRTrainer.train()` calls `compute_exploitability` every `es_validation_interval` steps using the current advantage network weights. Results are logged to the standard metrics stream and displayed in the live training display. Setting `es_validation_interval: 0` disables ES validation entirely.

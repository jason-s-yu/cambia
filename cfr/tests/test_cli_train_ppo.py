...
def test_train_ppo_help_names_the_default_opponent(runner, cambia_app):
    ...
    assert "imperfect_greedy" in result.stdout


def test_train_ppo_help_names_num_players(runner, cambia_app):
    result = runner.invoke(cambia_app, ["train", "ppo", "--help"])
    assert result.exit_code == 0
    assert "--num-players" in result.stdout


def test_train_ppo_defaults_opponent_to_imperfect_greedy(runner, cambia_app, tmp_path):
...
def test_train_ppo_accepts_num_players_in_range(
    runner, cambia_app, tmp_path, num_players
):
    with patch("src.ppo_train.train_ppo") as mock_train:
        result = runner.invoke(
            cambia_app,
...
def test_train_ppo_self_play_overrides_opponent(runner, cambia_app, tmp_path):

...
def _make_rebel_checkpoint(
    path: str, value_hidden_dim: int = 64, policy_hidden_dim: int = 32
):
...
def test_get_agent_rebel():
    ...
    agent = get_agent(
        "rebel", player_id=0, config=config, checkpoint_path=path, device="cpu"
    )
...
def test_imported_dims_match_expected_values():
    ...
    from src.pbs import (
        PBS_INPUT_DIM as actual_pbs_dim,
        NUM_HAND_TYPES as actual_hand_types,
    )
    ...
    assert (
        actual_hand_types == 468
    ), f"NUM_HAND_TYPES expected 468, got {actual_hand_types}"
    assert (
        actual_num_actions == 146
    ), f"NUM_ACTIONS expected 146, got {actual_num_actions}"

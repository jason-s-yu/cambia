...
def test_episode_sample_shapes(value_net, policy_net, fast_config):
    ...
    for i, s in enumerate(samples):
        ...
        assert (
            s.features.dtype == np.float32
        ), f"sample {i} features dtype {s.features.dtype}"
...
def test_value_targets_finite(value_net, policy_net, fast_config):
    ...
    for i, s in enumerate(samples):
        assert np.isfinite(
            s.value_target
        ).all(), f"sample {i} value_target contains non-finite values: {s.value_target}"
...
def test_features_finite(value_net, policy_net, fast_config):
    ...
    for i, s in enumerate(samples):
        assert np.isfinite(
            s.features
        ).all(), f"sample {i} features contains non-finite values"

...
# ===== validate_inputs flag =====


class TestValidateInputsFlag:
    def test_advantage_nan_raises_by_default(self):
        """AdvantageNetwork (default validate_inputs=True) raises on NaN features."""
        net = AdvantageNetwork()
        features = torch.randn(1, INPUT_DIM)
    ...
    def test_advantage_nan_skipped_when_disabled(self):
        """AdvantageNetwork(validate_inputs=False) does not raise on NaN features."""
        net = AdvantageNetwork(validate_inputs=False)
        net.eval()
        features = torch.randn(1, INPUT_DIM)
        features[0, 0] = float("nan")
        mask = torch.ones(1, NUM_ACTIONS, dtype=torch.bool)
        out = net(features, mask)  # Should not raise
        assert out.shape == (1, NUM_ACTIONS)

    def test_strategy_nan_raises_by_default(self):
        """StrategyNetwork (default validate_inputs=True) raises on NaN features."""
        net = StrategyNetwork()
        features = torch.randn(1, INPUT_DIM)
        features[0, 0] = float("nan")
        mask = torch.ones(1, NUM_ACTIONS, dtype=torch.bool)
    ...
    def test_strategy_nan_skipped_when_disabled(self):
        """StrategyNetwork(validate_inputs=False) does not raise on NaN features."""
        net = StrategyNetwork(validate_inputs=False)
        net.eval()
        features = torch.randn(1, INPUT_DIM)
        features[0, 0] = float("nan")
        mask = torch.ones(1, NUM_ACTIONS, dtype=torch.bool)
    ...
    def test_shape_validation_always_active(self):
        """Shape validation raises even when validate_inputs=False."""
        net = AdvantageNetwork(validate_inputs=False)
        net.eval()
        features = torch.randn(1, INPUT_DIM + 10)  # Wrong input_dim
        mask = torch.ones(1, NUM_ACTIONS, dtype=torch.bool)
        with pytest.raises(InvalidNetworkInputError):
            net(features, mask)

...
def build_scenarios(encode_fn, input_dim):
    ...
    def _st(
        ...
        discard,
        phase,
        stock=StockpileEstimate.HIGH.value,
        tp=0.05,
    ):
        slot_tags = (
            list(own_tags)
        ...
        f = encode_fn(
            ...
            stock,
            phase,
            DecisionContext.START_TURN.value,
            turn_progress=tp,
        )
    ...
    def _pd(
        ...
        f = encode_fn(
            ...
            slot_bkts,
            discard,
            StockpileEstimate.HIGH.value,
            phase,
            DecisionContext.POST_DRAW.value,
            drawn_card_bucket=drawn,
            turn_progress=tp,
        )
    ...
    def _pk(own_tags, own_buckets, opp_tags, opp_buckets, discard, phase, tp=0.25):
        ...
        f = encode_fn(
            ...
            StockpileEstimate.HIGH.value,
            phase,
            DecisionContext.ABILITY_SELECT.value,
            turn_progress=tp,
        )
    ...
    def _sn(own_tags, own_buckets, opp_tags, opp_buckets, discard, phase, tp=0.25):
        ...
        f = encode_fn(
            ...
            StockpileEstimate.HIGH.value,
            phase,
            DecisionContext.SNAP_DECISION.value,
            turn_progress=tp,
        )
    ...
    f, m = _st(
        [P, P, P, P],
        [B.ZERO.value, B.NEG_KING.value, B.ACE.value, B.ACE.value],
        [U, U, U, U],
        [0, 0, 0, 0],
        B.MID_NUM.value,
        GamePhase.EARLY.value,
    )
    scenarios.append(("Perfect(1) T1C", f, m, "P(Cambia)", lambda s, a: s[IDX_CAMBIA]))

    # S2: Terrible hand, should NOT Cambia
    f, m = _st(
        [P, P, P, P],
        [B.HIGH_KING.value, B.SWAP_BLIND.value, B.PEEK_OTHER.value, B.PEEK_OTHER.value],
        [U, U, U, U],
        [0, 0, 0, 0],
        B.MID_NUM.value,
        GamePhase.EARLY.value,
    ...
    f, m = _st(
        ...
        stock=StockpileEstimate.LOW.value,
        tp=0.6,
    )
    ...
    f, m = _st(
        ...
        B.LOW_NUM.value,
        GamePhase.MID.value,
        stock=StockpileEstimate.MEDIUM.value,
        tp=0.3,
    )
    ...
    scenarios.append(("Snap K blind", f, m, "P(Pass)", lambda s, a: s[IDX_PASS_SNAP]))
...
def load_ema(run_dir, device="cpu"):
    ...
    any_ckpt = glob.glob(
        os.path.join(run_dir, "checkpoints", "deep_cfr_checkpoint_iter_*.pt")
    )
    if not any_ckpt:
        return None, None
    cfg = torch.load(any_ckpt[0], map_location=device, weights_only=True).get(
        "dcfr_config", {}
    )
...
def evaluate_scenarios(net, scenarios, device="cpu"):
    ...
    for name, features, mask, metric_label, metric_fn in scenarios:
        ...
        with torch.inference_mode():
            ...
            strat = (
                get_strategy_from_advantages(raw.unsqueeze(0), mask_t.cpu())
                .squeeze(0)
                .numpy()
            )
            raw_np = raw.numpy()
...
def main():
    ...
    parser.add_argument(
        "run_dir",
        nargs="?",
        default="runs/interleaved-resnet-adaptive",
    ...
    if is_legacy:
        try:
            ...
            print(
                "Legacy encoding not available. Cannot run canonical tests on legacy checkpoints."
            )
            sys.exit(1)
    else:
    ...
    print(
        f"Checkpoints: {len(checkpoints)} ({checkpoints[0][0]} to {checkpoints[-1][0]})"
    )
    print()
    ...
    random_net = build_advantage_network(
        input_dim=input_dim,
        hidden_dim=first_cfg.get("hidden_dim", 256),
        output_dim=NUM_ACTIONS,
        dropout=0.1,
        validate_inputs=False,
    ...
    for label, results in all_results:
        ...
        n = len(results)
        print(
            f"  {label:<{name_w}}: {inversions}/{n} inversions "
    ...
    for label, results in all_results:
        ...
        perfect, terrible, good_late, unknown = (
            results[0],
            results[1],
            results[2],
            results[3],
        ...
        print(
            f"  {label:<{name_w}}: Perfect={perfect:.3f}  Good-late={good_late:.3f}  "

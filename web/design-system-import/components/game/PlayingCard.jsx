...
const GLYPHS = { spades: '♠', hearts: '♥', diamonds: '♦', clubs: '♣' };
...
export function PlayingCard({ rank, suit, faceDown = false, size = 'md', selected = false, onClick, style }) {
  const d = DIMS[size] || DIMS.md;
  const joker = rank === 'JOKER';
  const glyph = joker ? '★' : (GLYPHS[suit] || '');
  const color = joker ? 'var(--honey-600)' : (RED[suit] ? 'var(--suit-red)' : 'var(--suit-black)');
  const base = {
    width: d.w, height: d.h, flex: 'none', position: 'relative',
    ...
    transition: 'transform var(--dur-med) var(--ease-snap), box-shadow var(--dur-fast) var(--ease-out)',
  ...
  if (faceDown) {
    return (
      <div onClick={onClick} style={{
        ...
        backgroundImage: 'repeating-linear-gradient(45deg, transparent, transparent 5px, var(--card-back-line) 5px, var(--card-back-line) 6.5px), repeating-linear-gradient(-45deg, transparent, transparent 5px, var(--card-back-line) 5px, var(--card-back-line) 6.5px)',

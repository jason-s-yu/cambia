const { Button, Badge, StatusBadge, StatRow, Spinner, Input, Select } = window.Cambia_cc8727;
...
// Win-rate curve data (iteration, win rate vs greedy)
const CURVE = [[0,.31],[400,.44],[800,.50],[1200,.55],[1600,.58],[2000,.61],[2400,.635],[2800,.66],[3200,.687]];
const CURVE2 = [[0,.30],[400,.41],[800,.47],[1200,.50],[1600,.54],[2000,.55],[2400,.57],[2800,.585],[3200,.60]];

function MetricsChart() {
  const W = 560, H = 180, PL = 40, PB = 24;
  const x = (it) => PL + (it / 3200) * (W - PL - 10);
  const y = (v) => (H - PB) - ((v - 0.25) / 0.5) * (H - PB - 10);
    ...
    <svg width="100%" viewBox={'0 0 ' + W + ' ' + H} style={{ display: 'block' }}>
      {[0.3, 0.4, 0.5, 0.6, 0.7].map((v) => (
        <g key={v}>
          ...
          <text x={PL - 6} y={y(v) + 3} textAnchor="end" fontSize="9" fill="var(--text-tertiary)" fontFamily="var(--font-mono)">{Math.round(v * 100)}%</text>
        </g>
      ))}
      ...
      {[0, 800, 1600, 2400, 3200].map((it) => (
        <text key={it} x={x(it)} y={H - 8} textAnchor="middle" fontSize="9" fill="var(--text-tertiary)" fontFamily="var(--font-mono)">{it}</text>
      ))}
    </svg>
  );
}

function GpuBar({ label, pct, detail }) {
  return (
    <div style={{ flex: 1, minWidth: 150 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--text-2xs)', marginBottom: 4 }}>
        <span style={{ fontWeight: 800, letterSpacing: 'var(--tracking-caps)', textTransform: 'uppercase', color: 'var(--text-tertiary)' }}>{label}</span>
        <span style={{ fontFamily: 'var(--font-mono)', color: pct > 85 ? 'var(--berry-400)' : 'var(--text-secondary)' }}>{detail}</span>
      </div>
  ...
  return (
    <button onClick={onClick} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 'var(--text-sm)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.name}</span>
        <StatusBadge status={r.status} />
      </div>
      <div style={{ display: 'flex', gap: 10, marginTop: 4, fontSize: 'var(--text-2xs)', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>
...
function TrainingApp() {
  const [sel, setSel] = React.useState('prtcfr-prod-v4');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <div style={{ height: 'var(--topbar-h)', display: 'flex', alignItems: 'center', gap: 14, padding: '0 22px', background: 'var(--surface-card)', borderBottom: '1.5px solid var(--border-default)', flex: 'none' }}>
        ...
        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>admin · local hardware</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 18, padding: 18, flex: 1, alignItems: 'start', maxWidth: 1360, width: '100%', margin: '0 auto' }}>
        {/* Runs sidebar */}
        <window.Panel title={'Runs · ' + RUNS.length} style={{ padding: '12px 10px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            <h1 style={{ margin: 0, fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: 'var(--text-xl)' }}>{run.name}</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16, alignItems: 'start' }}>
            <window.Panel title="Win rate vs baselines" action={<span style={{ fontSize: 'var(--text-2xs)', fontFamily: 'var(--font-mono)' }}><span style={{ color: 'var(--ember-400)' }}>■ vs greedy</span>&nbsp;&nbsp;<span style={{ color: 'var(--dusk-400)' }}>■ vs random-plus</span></span>}>
              <MetricsChart />
            </window.Panel>
            <window.Panel title="Summary">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
            <window.Panel title="Checkpoints">
              {CHECKPOINTS.map((c, i) => (
                <div key={c.iter} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 2px', borderTop: i ? '1px solid var(--border-subtle)' : 'none' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 700 }}>ckpt_{c.iter}</span>
                  {c.best && <Badge tone="gold">BEST</Badge>}
                  <span style={{ flex: 1 }}></span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', color: 'var(--text-secondary)' }}>{c.size}</span>
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)', width: 70, textAlign: 'right' }}>{c.when}</span>
                </div>
              ))}
            </window.Panel>
            <window.Panel title="Eval jobs">
              {EVALS.map((e, i) => (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 2px', borderTop: i ? '1px solid var(--border-subtle)' : 'none' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 'var(--text-sm)', fontWeight: 700 }}>{e.id}</span>
                  <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', flex: 1 }}>vs {e.target} · {e.games} games · {e.device}</span>
          ...
          <window.Panel title="Live log · runs/prtcfr-prod-v4/train.log">
            <div style={{ background: 'var(--surface-inset)', border: '1.5px solid var(--border-subtle)', borderRadius: 'var(--radius-md)', padding: '10px 14px', fontFamily: 'var(--font-mono)', fontSize: 'var(--text-xs)', lineHeight: 1.7, color: 'var(--text-secondary)', overflowX: 'auto', whiteSpace: 'pre' }}>
              {LOG_LINES.join('\n')}
            </div>
          </window.Panel>
        </div>

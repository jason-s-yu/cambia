...
export interface OutboundRecord {
	type: string;
	/** Card ids the frame names; all of them must still exist for a resend. */
	cardRefs: string[];
...
/** Card ids named by a client action frame. */
...
export function tableContext(
	...
	if (gs) {
		for (const p of gs.players) {
			for (const c of p.revealedHand ?? []) cardIds.push(c.id);
...
export function decideResend(rec: OutboundRecord, now: TableContext, syncSeq: number): ResendVerdict {
	...
	// Every card the frame names must still be somewhere this client can see it.
	if (rec.cardRefs.some((id) => !now.cardIds.includes(id))) return 'notify';
	...
	switch (rec.type) {
		case 'action_snap':
			...
			// discard top is the snap's whole subject: if it changed, the same card id now means
			// a different move.
			if (!sameTurn) return 'notify';
			if (!now.discardTopId || now.discardTopId !== rec.ctx.discardTopId) return 'notify';
			if (now.pendingAction !== rec.ctx.pendingAction) return 'notify';
			return 'resend';

		case 'action_draw_discardpile':
			// Taking the discard is bound to the card that was on top when it was clicked.
			if (!now.discardTopId || now.discardTopId !== rec.ctx.discardTopId) return 'notify';

...
Submission runs a fixed validation order: name shape -> name collision -> kind allowlist (`train`, `evaluate`, `head-to-head`, `bench`, `measure`; v1 `train` only launches the `prtcfr` algorithm) -> device shape (`cpu`, `cuda`, or `xpu`) -> device capability gate (`RUNNERD_ALLOWED_DEVICES`) -> lexical path guards on `config`/`checkpoint_*`/`target`/`warm_start`/`script`/`reads` (reject absolute paths and `..` segments) -> containment guards (checkpoints, an evaluate target, a train job's `warm_start`, and a measure job's `reads` must resolve inside the runs directory) -> resource preflights (disk, RAM, and a device-routed GPU check).

A `train` job may optionally set `warm_start` to another run's staged snapshot, relative to the runs directory (e.g. `prior-run/snapshots/prtcfr_snapshot_iter_530.pt`), to initialize from it instead of starting cold; `evaluate`, `head-to-head`, `bench`, and `measure` forbid the field (`400 invalid_warm_start`), and a `warm_start` that resolves but names a file that does not exist is rejected at submit (`400 warm_start_not_found`) rather than failing the job after it launches.

A `measure` job runs a pinned script instead of a `cambia` subcommand: `script` must be a worktree-relative path under `cfr/scripts/` (`400 invalid_script` otherwise, and a script absent at the pinned commit fails the job at launch), `args` is its argv tail passed verbatim with no shell (each entry capped at 4096 bytes and rejected on a NUL byte), and `reads` names other run directories exported read-only as `CAMBIA_MEASURE_READ_DIRS` (a nonexistent read is rejected at submit). `script`/`args`/`reads` are forbidden on every other kind. `CAMBIA_RUN_DB` points at the measure job's own run dir, unlike `evaluate`'s borrowed target journal.

Launch argv by kind: `train` runs `cambia train prtcfr --config <rendered> --run-name <name> --save-path runs/<name>`; `evaluate` runs `cambia evaluate <resolved target> --latest --games N --device D` (no `--config`: run-dir mode reads the target's own `config.yaml`); `head-to-head` runs `cambia head-to-head --config <rendered> --checkpoint-a <resolved> --checkpoint-b <resolved> --games N --device D` (`config` is required for this kind, unlike evaluate, since two bare checkpoints carry no rules/agent-type of their own); `bench` runs `cambia benchmark all --config <rendered> --output-dir runs/<name> --device D`; `measure` runs `<staged script path> <args...>` directly, with no `-m src.cli` prefix and no `cambia` subcommand.

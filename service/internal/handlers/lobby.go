...
func CreateLobbyHandler(gs *GameServer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if reqBody != nil {
			...
			// sending the sheet field by field. lob.Update expands it below, but this handler
			// has always discarded that error, so the id is checked up front to get a 400 out
			// of a bad one, and the game mode the preset fixes is derived here because a preset
			// is picked before there is a lobby to update. A queued or ranked lobby refuses a
			// preset for the same reason its host cannot edit rules at all: the queue owns them.
			if presetID, ok := reqBody["presetId"].(string); ok && presetID != "" {
				if lob.Type == "matchmaking" || lob.QueueID != "" || lob.Mode == "ranked" {

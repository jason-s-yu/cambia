...
func startTwoPlayerGame(t *testing.T) (*game.CambiaGame, uuid.UUID, uuid.UUID, *wsTestClient, *wsTestClient) {
	...
	// teardown. t.Cleanup is LIFO, so registering this here puts it after the host/p2 close
	// cleanups registered below (it runs once those sockets have actually closed, so there is
	// something to wait for) and before createPublicLobby's cleanupLobbyDBRows registered just
	// above, whose DELETE must not run while a write is still in flight - registering it ahead
	// of createPublicLobby, as this originally did, inverted that and let the DELETE go first
	// (cambia-942 F4). newForfeitTestServer's own t.Cleanup(ts.Close), registered before this
	// function ran, still tears down the test server last of all.
	//
	// It polls for the resulting forfeit's GameOver rather than calling awaitGameEndPersistence
	// directly: GameOver is set under the same lock endGame holds for its whole call, including
	// persistFinalGameState's WaitGroup.Add, so observing it proves that Add already ran and
	// this can never Wait ahead of it (cambia-908; see awaitGameEndPersistence for the full
	// ordering rule).
